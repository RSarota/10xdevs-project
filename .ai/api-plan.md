...
- **Error Responses:**
  ...
  - `400 Bad Request`: Validation error
    - front > 200 chars or back > 500 chars
    - AI flashcards: generation_id not found or doesn't belong to user
    - Bulk: empty flashcards array or more than 100 flashcards
...
4. **Success Response:**
   ...
   - Client manages proposals locally (e.g., by array index)

5. **Acceptance Flow:**
   - Client uses `POST /api/flashcards` to accept flashcards:
     - **Single flashcard**: Send individual flashcard object with `source` and `generation_id`
     - **Bulk operation**: Send array of flashcards for optimized batch insert
