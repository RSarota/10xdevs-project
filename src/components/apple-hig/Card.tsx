...
/**
 * Card Image - For media content
 */
export interface CardImageProps extends React.ImgHTMLAttributes<HTMLImageElement> {
  aspectRatio?: "square" | "video" | "wide" | "portrait";
}

export const CardImage: React.FC<CardImageProps> = ({ aspectRatio = "video", className = "", alt = "", ...props }) => {
  const aspectClasses = {
  ...
  return (
    <div
      className={`relative ${aspectClasses[aspectRatio]} overflow-hidden -m-[var(--apple-space-6)] mb-[var(--apple-space-6)]`}
    >
      <img className={`absolute inset-0 w-full h-full object-cover ${className}`} alt={alt} {...props} />
    </div>
  );
};

CardImage.displayName = "AppleCardImage";

/**
 * Grouped Card Container - For list-style layouts with inset cards
 */
export interface CardGroupProps extends React.HTMLAttributes<HTMLDivElement> {
  spacing?: "sm" | "md" | "lg";
}

export const CardGroup: React.FC<CardGroupProps> = ({ spacing = "md", className = "", children, ...props }) => {
  const spacingClasses = {
    sm: "space-y-[var(--apple-space-3)]",
    md: "space-y-[var(--apple-space-5)]",
    lg: "space-y-[var(--apple-space-7)]",
  ...
  return (
    <div
      className={`
        ...
        p-[var(--apple-space-5)]
        bg-[hsl(var(--apple-grouped-bg))]
        rounded-[var(--apple-radius-xlarge)]
        ${className}
      `
        .trim()
        .replace(/\s+/g, " ")}
...
CardGroup.displayName = "AppleCardGroup";
